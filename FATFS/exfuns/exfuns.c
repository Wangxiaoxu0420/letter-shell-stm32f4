...
u8 exfuns_init(void)
{
	...
	{
		fs[i]=(FATFS*)ff_memalloc(sizeof(FATFS));	//为磁盘i工作区申请内存	
		if(!fs[i])break;
	}
	file=(FIL*)ff_memalloc(sizeof(FIL));		//为file申请内存
	ftemp=(FIL*)ff_memalloc(sizeof(FIL));		//为ftemp申请内存
	fatbuf=(u8*)ff_memalloc(512);				//为fatbuf申请内存
...
u8 exf_copy(u8(*fcpymsg)(u8*pname,u8 pct,u8 mode),u8 *psrc,u8 *pdst,u32 totsize,u32 cpdsize,u8 fwmode)
{
	...
	unsigned long long lcpdsize=cpdsize; 
 	fsrc=(FIL*)ff_memalloc(sizeof(FIL));//申请内存
 	fdst=(FIL*)ff_memalloc(sizeof(FIL));
	fbuf=(u8*)ff_memalloc(8192);
...
#define MAX_PATHNAME_DEPTH	512+1	//最大目标文件路径+文件名深度
	...
	fddir=(DIR*)ff_memalloc(sizeof(DIR));//申请内存
...
#define MAX_PATHNAME_DEPTH	512+1	//最大目标文件路径+文件名深度
	...
	srcdir=(DIR*)ff_memalloc(sizeof(DIR));//申请内存
